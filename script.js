...
const CP = ['#1a2540','#2563eb','#0891b2','#16a34a','#d97706','#dc2626','#7c3aed','#db2777','#065f46','#92400e'];
...
function showPage(page, el) {
  ...
  setTimeout(function(){ renderAll(); }, 30);
}
...
function init() {
  ...
  [...new Set(RAW.forecast.map(r=>r.producto))].sort().forEach(p=>{
    const o=document.createElement('option');
    o.value=p; o.textContent=p.length>26?p.slice(0,26)+'…':p;
    pf.appendChild(o);
  });
...
function renderProduccion() {
  ...
  mkChart('chart-unidades','bar',{
    ...
    datasets:[
      {label:'Capacidad', data:ef.map(r=>r.capacidad), backgroundColor:'rgba(26,37,64,0.12)', borderColor:'#1a2540', borderWidth:1.5, borderRadius:3},
      {label:'Planificado', data:ef.map(r=>r.planificada), backgroundColor:'rgba(37,99,235,0.25)', borderColor:'#2563eb', borderWidth:1.5, borderRadius:3},
      {label:'Efectivo', data:ef.map(r=>r.efectiva), backgroundColor:'rgba(8,145,178,0.55)', borderColor:'#0891b2', borderWidth:1.5, borderRadius:3},
    ]
  }, barOptsWithLabels());
...
function renderAnualBarras() {
  ...
  mkChart('chart-anual-barras','bar',{
    labels:meses,
    datasets:[
      {label:'Capacidad',data:meses.map(m=>byMes[m].cap),backgroundColor:'rgba(26,37,64,0.12)',borderColor:'#1a2540',borderWidth:1.5,borderRadius:3},
      {label:'Planificado',data:meses.map(m=>byMes[m].plan),backgroundColor:'rgba(37,99,235,0.25)',borderColor:'#2563eb',borderWidth:1.5,borderRadius:3},
      {label:'Efectivo',data:meses.map(m=>byMes[m].ef),backgroundColor:'rgba(8,145,178,0.5)',borderColor:'#0891b2',borderWidth:1.5,borderRadius:3},
    ]
  }, barOptsWithLabels());
}
...
function renderForecastLinea() {
  ...
  const byMes = {};
  RAW.forecast.filter(r=>r.producto===sel).forEach(r=>{ byMes[r.mes]=(byMes[r.mes]||0)+r.produccion; });
  const meses = MESES.filter(m=>byMes[m]);
  mkChart('chart-anual-linea','line',{
    labels:meses.length?meses:[currentMes],
    datasets:[{label:'Producción',data:meses.length?meses.map(m=>byMes[m]||0):[byMes[currentMes]||0],borderColor:'#1a2540',backgroundColor:'rgba(26,37,64,0.08)',tension:.3,pointRadius:4,pointBackgroundColor:'#1a2540',fill:true}]
  },{
    responsive:true,maintainAspectRatio:false,
...
function renderKpiLogCards() {
  ...
  el.innerHTML = kpisOrdered.map(r=>{
    const isPct = (r.valor<2&&r.meta&&r.meta<2) || (r.valor>2&&r.indicador.toLowerCase().includes('exactitud')||r.indicador.toLowerCase().includes('picking'));
    const isAlreadyPct = r.valor > 2 && (r.indicador.toLowerCase().includes('exactitud') || r.indicador.toLowerCase().includes('picking'));
    const ok = r.meta ? (isAlreadyPct ? r.valor>=r.meta*100*0.98 : isPct ? r.valor>=r.meta*0.98 : r.valor<=r.meta*1.1) : true;
    const val = isAlreadyPct ? r.valor+'%' : isPct ? fmtPct(r.valor) : r.valor+(r.indicador.includes('Días')||r.indicador.includes('Dias')?' días':'');
    const metaStr = r.meta ? (isAlreadyPct ? 'Meta '+(r.meta*100)+'%' : isPct ? 'Meta '+fmtPct(r.meta) : 'Meta '+r.meta.toFixed(0)+'d') : '';
...
function renderPedidosChart() {
  ...
  const almColors = {'Granger S.A.':'#1a2540','ENBOX':'#2563eb','MELI FULL':'#0891b2'};
...
function renderEvolPedidos() {
  const MESES_ORD = ['Enero','Febrero','Marzo','Abril','Mayo'];
  const byMes = RAW.log_summary.by_mes;
  ...
  if(evolCanales.B2B) datasets.push({
    ...
    borderColor: '#1a2540', backgroundColor: 'rgba(26,37,64,0.06)',
    tension: 0.35, fill: true, pointRadius: 5, pointHoverRadius: 7,
    pointBackgroundColor: '#1a2540', borderWidth: 2.5
  });
...
function renderTiemposB2C() {
  ...
  document.getElementById('tabla-tiempos-oca').innerHTML=
    `<thead><tr><th>Provincia</th><th class="td-num">Días</th></tr></thead><tbody>`+
    oca.map(r=>`<tr><td>${r.provincia}</td><td class="td-num" style="color:${colorDias(r.tiempo)}">${typeof r.tiempo==='number'?r.tiempo.toFixed(1):r.tiempo}</td></tr>`).join('')+`</tbody>`;

  document.getElementById('tabla-tiempos-enbox').innerHTML=
    `<thead><tr><th>Zona</th><th class="td-num">Tiempo</th></tr></thead><tbody>`+
    enbox.map(r=>`<tr><td>${r.provincia}</td><td class="td-num">${r.tiempo}</td></tr>`).join('')+`</tbody>`;
}
...
function renderCompras() {
  ...
  const barColors=['#1a2540','#2563eb','#0891b2','#16a34a'];
